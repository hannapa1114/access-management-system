...
const { switchQueryLog, switchQueryOut } = require('../../query/func')
...
module.exports = {
  post: async (req, res) => {
    ...
    const { company, representName, createdAt, updatedAt } = fields;
    await oracleDb.getConnection({
      user: 'datausr',
      password: 'nccrekr1!',
      connectString: '10.32.29.204:1521/kccsisdb',
    }, async (err, connection) => {
      await connection.execute(switchQueryOut(query.out, company, representName, createdAt, updatedAt), async function (err, result) {
          if (err) {
            console.error(err)
            return;
          }
